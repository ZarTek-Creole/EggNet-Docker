...
if [ ! -f "${EGG_PATH_TLS}/pem/${EGG_LONG_NAME}.pem" ] || [ $EGG_TLS_REGENERATE == 1 ]
then
    rm -rf ${EGG_PATH_TLS}/pem/${EGG_LONG_NAME}.pem
    openssl ecparam -genkey -name prime256v1 -out ${EGG_PATH_TLS}/pem/${EGG_LONG_NAME}.pem
fi

if [ ! -f "${EGG_PATH_TLS}/key/${EGG_LONG_NAME}.key" ] || [ $EGG_TLS_REGENERATE == 1 ]
then
    rm -rf ${EGG_PATH_TLS}/key/${EGG_LONG_NAME}.key
    openssl genrsa -out ${EGG_PATH_TLS}/key/${EGG_LONG_NAME}.key 4096
fi

if [ ! -f "${EGG_PATH_TLS}/crt/${EGG_LONG_NAME}.crt" ] || [ $EGG_TLS_REGENERATE == 1 ]
then
    rm -rf ${EGG_PATH_TLS}/crt/${EGG_LONG_NAME}.crt
