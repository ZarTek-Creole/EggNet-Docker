...
proc userfileloaded { args } {
	...
	if { [validuser ${::env(PPL_USER)}] != 1 } {
		try {
			...
			putlog "Ajout du user-master: ${::env(PPL_USER)} réussi"
		}
	}
...
proc ::BOTNET_CREATE {} {
	foreach FILE_SCT [glob -directory ${::EGG_PATH_SECRETS} *.sct] {
		putlog "::BOTNET_CREATE bots: [bots]"
		set B_NAME [file rootname [file tail ${FILE_SCT}]]
		if { "${::botnet-nick}" == "${B_NAME}" || [validuser ${B_NAME}] } { continue }
		set FILEPIPE		[open "${FILE_SCT}" r]
		set SCT_DATA 		[split [read ${FILEPIPE}]]
		close ${FILEPIPE}
		...
		putlog "debug : BOT_ISMASTER ${BOT_ISMASTER} BOT_HOSTNAME ${BOT_HOSTNAME}"
		...
		if {$BOT_ISMASTER == 1 && $::EGG_ISMASTER == 0} {
			...
			putlog "aucune action pour ${B_NAME}"
		}
		catch {link ${B_NAME}}
	}
      catch {save}
